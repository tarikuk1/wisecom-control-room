...
const LOGIN_HTML = `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Control Room — Connexion</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{background:#0c0c0c;font-family:'Segoe UI',sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;}.card{background:#111;border:1px solid #1e1e1e;border-radius:14px;padding:40px;width:360px;box-shadow:0 24px 60px rgba(0,0,0,.6);}.logo{display:flex;align-items:center;gap:10px;margin-bottom:32px;}.dot{width:10px;height:10px;border-radius:50%;background:#E8006E;box-shadow:0 0 12px #E8006E;}.brand{font-weight:800;font-size:15px;letter-spacing:.07em;color:#fff;}.brand-sub{font-size:10px;color:#555;margin-top:1px;}label{display:block;font-size:11px;color:#666;font-weight:600;text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px;}input{width:100%;background:#161616;color:#fff;border:1px solid #2a2a2a;border-radius:8px;padding:11px 14px;font-size:14px;outline:none;margin-bottom:16px;transition:border .15s;}input:focus{border-color:#E8006E;}button{width:100%;background:linear-gradient(135deg,#E8006E,#c0005a);color:#fff;border:none;border-radius:8px;padding:13px;font-size:14px;font-weight:700;cursor:pointer;}.error{background:#1a0808;border:1px solid #5a1010;color:#ff6b6b;border-radius:6px;padding:10px 14px;font-size:12px;margin-bottom:16px;display:ERRORDISPLAY;}.footer{text-align:center;font-size:10px;color:#333;margin-top:24px;}</style></head><body><div class="card"><div class="logo"><div class="dot"></div><div><div class="brand">CONTROL ROOM</div><div class="brand-sub">WISECOM · ACCÈS SÉCURISÉ</div></div></div><div class="error">Identifiants incorrects.</div><form method="POST" action="/login"><label>Identifiant</label><input type="text" name="login" placeholder="Votre login" required><label>Mot de passe</label><input type="password" name="password" placeholder="••••••••" required><button type="submit">Se connecter</button></form><div class="footer">Wisecom © ${new Date().getFullYear()} · Accès réservé</div></div></body></html>`;
...
const server=http.createServer(async(req,res)=>{
  ...
  if(url==="/login"&&req.method==="GET"){res.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});return res.end(LOGIN_HTML.replace("ERRORDISPLAY","none"));}
  if(url==="/login"&&req.method==="POST"){
    ...
    req.on("end",()=>{
      ...
      if(USERS[login]&&USERS[login]===pwd){const t=createSession(login);setCookie(res,"session",t);res.writeHead(302,{Location:"/"});console.log(`[AUTH] Connexion: ${login}`);}
      else{res.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});res.end(LOGIN_HTML.replace("ERRORDISPLAY","block"));}
      res.end();
    });return;
  }
  if(url==="/logout"){if(cookies.session)delete sessions[cookies.session];setCookie(res,"session","",0);res.writeHead(302,{Location:"/login"});return res.end();}
